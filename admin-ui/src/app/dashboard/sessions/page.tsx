            ...
            {sessions.map((s) => (
              <tr key={s.id} className="hover:bg-zinc-800/50">
                <td className="px-4 py-3">
                  <p className="text-white">{s.memberUser.name ?? '—'}</p>
                  <p className="text-zinc-500 text-xs">{s.memberUser.email}</p>
                </td>
                <td className="px-4 py-3 text-zinc-300">{s.trainerProfile.displayName}</td>
                <td className="px-4 py-3 text-zinc-300 whitespace-nowrap">{fmt(s.startsAt)}</td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                    s.status === 'SCHEDULED' ? 'bg-blue-900 text-blue-300' :
                    s.status === 'CANCELLED_BY_MEMBER' ? 'bg-yellow-900 text-yellow-300' :
                ...
                <td className="px-4 py-3 text-zinc-400">
                  {s.attendance ? (
                    <span className={`px-2 py-0.5 rounded-full text-xs ${
                      s.attendance.outcome === 'ATTENDED' ? 'bg-green-900 text-green-300' :
                      s.attendance.outcome === 'MISSED' ? 'bg-red-900 text-red-300' :
                ...
                <td className="px-4 py-3 flex gap-3 items-center">
                  ...
                  {s.status === 'SCHEDULED' && s.startsAt <= now && !s.attendance && (
                    <form action={markAttendance} className="flex gap-2 items-center">
                      ...
                      <select name="outcome" className="text-xs bg-zinc-800 border border-zinc-700 rounded px-2 py-1 text-zinc-300">
