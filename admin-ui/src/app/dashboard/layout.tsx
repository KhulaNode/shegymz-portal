    ...
    <div className="flex min-h-screen">
      <aside className="w-56 bg-zinc-900 border-r border-zinc-800 flex flex-col">
        <div className="px-5 py-5 border-b border-zinc-800">
          ...
          <p className="text-pink-400 text-xs mt-0.5">{isAdmin ? 'Admin Panel' : 'Trainer Portal'}</p>
        </div>
        ...
        <div className="px-4 py-4 border-t border-zinc-800">
          ...
          <p className="text-xs text-zinc-600 mb-2 truncate">{session.user.email}</p>
          <span className={`inline-block text-xs px-2 py-0.5 rounded-full mb-2 ${isAdmin ? 'bg-pink-900 text-pink-300' : 'bg-zinc-700 text-zinc-300'}`}>
            {session.user.role}
          </span>
          <Link href="/api/auth/signout" className="block text-xs text-zinc-400 hover:text-white">
            Sign out
          </Link>
        </div>
      </aside>
